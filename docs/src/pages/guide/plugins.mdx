...
[rss]: /guide/rss
[îles docs configuration]: https://github.com/ElMassimo/iles/blob/main/docs/iles.config.ts
...
A module to add and configure [vite-plugin-pwa] to make your app work offline.
...
You can take a look at the source code of this website using `generateSW` strategy and `prompt` for update:
- [îles docs configuration]
- [ReloadPrompt component]

<Tip title="Register service worker generated with generareSW strategy with auto update">
To register the service worker generated using `generateSW` strategy with `autoUpdate` add these client entries to the base layout on your application:
...
You can still configure [vite-plugin-pwa] as a `Vite` plugin instead using [@islands/pwa] module: to make your app work offline you will also need to add [@islands/pwa] module.

In the case you want to configure [vite-plugin-pwa] as a `Vite` plugin, [@islands/pwa] module will use it, you will need to configure its options properly.
...
export default defineConfig({
  ...
  modules: [
    pwa({pwa options will be ignored by the module}),
  ],
  vite: {
    plugins: [VitePWA({pwa options})]
  }
  ...
})
```
