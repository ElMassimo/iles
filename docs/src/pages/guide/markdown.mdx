...
<Iles /> provides excellent support for using [components][frameworks] inside markdown (`.mdx`), as
well as using markdown components inside Vue.
...
<Iles /> provides excellent support for using components inside markdown (`.mdx`), as well as using
markdown components inside Vue.
```
...
<Tip title={title}>{description}</Tip>
```
...
<Iles /> extends [MDX] enabling you to use [components from different frameworks][frameworks] in MDX
files.
...
```mdx
<Tip title="Markdown in Slots">This is a _markdown-based_ comment. **Great!**</Tip>
```
...
export const titleWithMarkdown = (
  <span>
    But better keep it <b>simple</b>
  </span>
);
