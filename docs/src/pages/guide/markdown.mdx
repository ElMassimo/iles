...
If you need to provide several slots to a component in [MDX], you can use the standard Vue syntax. `v-slots` or [object slots][vue-jsx] are not supported.

```mdx
<Tip warn>
  <template #title>
    <span>But better keep it <b>simple</b></span>
  </template>
  At this point you might be better off using a Vue page instead.
</Tip>
```
