...
[named slots]: https://v3.vuejs.org/guide/component-slots.html#named-slots
...
<Tip title="Directives">
You can use directives such as `v-if` and `v-for` (unlike in [Vue JSX]).

The shortcuts for `v-bind`, `v-on`, and `v-slot` are not valid MDX, so use them __explicitly__.
</Tip>
...
You can use [named slots] as usual, although it's usually nicer to extract into
components to keep the MDX simpler (and framework-agnostic).

```mdx
<Tip>
  <template v-slot:title>
    <b>Table of Contents</b> _(just an example, use a Vue component instead)_
  </template>
  <ul>
    <li v-for="heading in headings.filter(h => h.level === 2)">
...
If you are used to [vue-jsx], have in mind [object slots][vue-jsx] are not supported.
