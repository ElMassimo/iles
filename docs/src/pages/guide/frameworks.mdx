...
<Tip warn>
  You must add a [hydration directive][hydration] when using non-Vue components in Vue and [MDX
  files].
</Tip>
...
<Tip title="But that's just what one benchmark concluded!" warn>
  Do your own measurements, and as mentioned previously, there are often more important factors than
  bundle size. Choose according to your project's circumstances.
</Tip>
