...
<Tip warn>
  Client script blocks are completely detached from the Vue component, which will be statically
  pre-rendered. The [strategy] is applied to the script, __not__ the component.
</Tip>
...
<Tip warn>You must provide a `client:` [strategy] when using vanilla components.</Tip>
