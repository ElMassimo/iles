...
- Create a Buddy account [here](https://buddy.works).
...
- Add a new pipeline.
- In the newly created pipeline add a [Node.js](https://buddy.works/actions/node-js) action.
- In this action add:
```
npm install
npm run build
...
- Add a deployment action — there are many to choose from, you can browse them [here](https://buddy.works/actions). Although their settings can differ, remember to set the Source path to dist.
