...
// @ts-ignore
import { HtmlTag, detach, empty, insert } from 'svelte/internal'
...
export default function createIsland(component: Component, id: string, el: Element, props: Props, slots: Slots | undefined = {}) {
  ...
  if (import.meta.env.DISPOSE_ISLANDS) { onDispose(id, app.$destroy) }

  if (import.meta.env.DEV) { (window as any).__ILE_DEVTOOLS__?.onHydration({ id, el, props, slots, component, framework: 'svelte' }) }
}
