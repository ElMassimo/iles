...
export function provideMDXComponents(mdxComponents: MDXComponents) {
  ...
  if (overrideElements) {
    for (const key in mdxComponents) {
      if (key === key.toLowerCase() && !overrideElements.includes(key)) { console.warn(`Provided an MDX shortcode for '${key}', but it's being optimized. You must specify '${key}' in mdxComponents in app.ts`) }
    }
  }
