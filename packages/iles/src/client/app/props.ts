  ...
  if (!route.meta.pathVariants) {
    ...
    watch(route.meta.pathVariantsPromise, async (promise) => {
      ...
      if (
        ...
        pathVariants.length !== route.meta.pathVariants!.value.length
      )
...
async function getPathVariants(route: RouteLocationNormalizedLoaded) {
  try {
    ...
    if (page.getStaticPaths && Object.keys(route.params).length === 0)
      console.warn(
        `getStaticPaths provided in ${page.filename || route.path}, but path is not dynamic.`,
      )

    const pathVariants = (await page.getStaticPaths?.({ route })) || []
    if (!Array.isArray(pathVariants))
      throw new Error(
        `Expected array from 'getStaticPaths' in ${page.filename}, got ${JSON.stringify(pathVariants)}`,
      )
