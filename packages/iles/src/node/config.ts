...
import { loadConfigFromFile, mergeConfig as mergeViteConfig, PluginOption } from "vite-plus"
...
async function resolveUserConfig(root: string, configEnv: ConfigEnv) {
  ...
  if ((userConfig as any).plugins)
    throw new Error(
      `îles 'plugins' have been renamed to 'modules'. If you want to provide Vite plugins instead, place them in 'vite:'. Received 'plugins' in ${(userConfig as any).configPath}:\n${JSON.stringify((userConfig as any).plugins)}`,
    )

  config.modules = compact<IlesModule>(
    await resolveIlesModules([
      {
        name: "iles:base-config",
        ...appConfigDefaults(config, userConfig as UserConfig, configEnv),
      },
      mdx(),
...
async function setNamedPlugins(config: AppConfig, env: ConfigEnv, plugins: NamedPlugins) {
  const ceChecks = config.modules
    .map((mod) => mod.vue?.template?.compilerOptions?.isCustomElement)
    .filter((x) => x)
  ...
  const optionalPlugins = {
    async solid(options: SolidOptions) {
      const solid =
        await importLibrary<(typeof import("vite-plugin-solid"))["default"]>("vite-plugin-solid")
      return solid({ ssr: true, ...options })
    },
    async preact(options: PreactOptions) {
      const preact =
        await importLibrary<(typeof import("@preact/preset-vite"))["default"]>(
          "@preact/preset-vite",
        )
      return preact(options)
    },
    async svelte(options: SvelteOptions) {
      const { svelte } = await importLibrary<typeof import("@sveltejs/vite-plugin-svelte")>(
      ...
      return svelte({
        emitCss: true,
        ...options,
        compilerOptions: { dev, ...options.compilerOptions },
      })
    },
  }
...
async function applyModules(config: AppConfig, configEnv: ConfigEnv) {
  for (const mod of config.modules) {
    if ((mod as any).modules && (mod as any).modules.length > 0)
      throw new Error(
        `Modules in îles can't specify the 'modules' option, return an array of modules instead. Found in ${mod.name}: ${JSON.stringify((mod as any).modules)}`,
      )
...
function viteConfigDefaults(root: string, userConfig: UserConfig): ViteOptions {
  return {
    ...
    define: {},
    optimizeDeps: {
      include: ["vue", "vue-router", "@unhead/vue", "@vue/devtools-api"],
      exclude: [
        "iles",
        "@nuxt/devalue",
        "@islands/hydration",
