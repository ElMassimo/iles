...
export default function IslandsPlugins (appConfig: AppConfig): PluginOption[] {
  ...
  const resolveIdFilter = new RegExp(`^(?:${
    [ILES_APP_ENTRY, APP_CONFIG_REQUEST_PATH, USER_APP_REQUEST_PATH, USER_SITE_REQUEST_PATH, NOT_FOUND_REQUEST_PATH, defaultLayoutPath]
      .map(escapeRegex)
      .join('|')
  })$`)
        ...
        filter: {
          id: {
            include: [
              ...
              new RegExp(escapeRegex(defaultLayoutPath)),
            ],
          },
        },
        async handler (id) {
