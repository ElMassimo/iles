...
function stringifyRoutes(routes: UserRoute[]) {
  return JSON.stringify(routes.map(toVueRouter)).replace(
    /"componentFilename":"(.*?)"/g,
    (_, componentPath) => `component: () => import('${componentPath}')`,
  )
}

function toVueRouter({
  frontmatter,
  templateAttrs,
  ...route
}: UserRoute & { templateAttrs?: Record<string, any> }) {
